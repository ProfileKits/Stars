...
import com.predictor.library.utils.CNCustomTypefaceSpan;


/**
 * 自定义文本控件
 * 例如： CNTextTool.getBuilder("").setContext(context).setBold().setAlign(Layout.Alignment.ALIGN_CENTER)
 *                 .append(dataBean.getWho() != null ? dataBean.getWho() : "" + "").setTypefaceHNDB().setForegroundColor(Color.WHITE).setBold()
 *                 .append(" ")
 *                 .append(dataBean.getPraised() != null ? dataBean.getPraised() : "").setTypefaceHND().setForegroundColor(Color.WHITE).setNoBold()
 *                 .append(" ")
 *                 .append(dataBean.getMe() != null ? dataBean.getMe() : "").setTypefaceHNDB().setForegroundColor(Color.WHITE).setBold()
 *                 .append("     ")
 *                 .append(dataBean.getDate() != null ? dataBean.getDate() : "").setTypefaceHND().setForegroundColor(Color.parseColor("#67FFFFFF")).setNoBold()
 *                 .into(holder.tv_praised);
 *
 *                 同一段文字显示不同颜色及样式
 */
public class CNTextTool {
    ...
    public static class Builder {
        ...
        private boolean isTypeface_hndb;
        private boolean isTypeface_hnd;
        ...
        public Builder setTypefaceHNDB() {
            isTypeface_hndb = true;
            return this;
        }


        public Builder setTypefaceHND() {
            isTypeface_hnd = true;
            return this;
        }
...
//            if (isTypeface_hnd) {
//                Typeface mCustomFont = LavaApplication.getInstance().getHelveticaNowDisplayLight(context);
//                mBuilder.setSpan(new CNCustomTypefaceSpan(mCustomFont), start, end, flag);
//                isTypeface_hnd = false;
//            }
//
//            if (isTypeface_hndb) {
//                Typeface mCustomFont = LavaApplication.getInstance().getHelveticaNowDisplayBold(context);
//                mBuilder.setSpan(new CNCustomTypefaceSpan(mCustomFont), start, end, flag);
//                isTypeface_hndb = false;
//            }
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(new ImageSpan(CNBaseTools.getContext(), bitmap), start, end, flag);
                    bitmap = null;
                    imageIsBitmap = false;
