...
import com.predictor.library.base.AppInfoMgr;
 ...
 * 对字符串按照常用规则进行验证的工具类
 */
public abstract class CNValidatorUtil {
    /**
     * 简体中文的正则表达式。
     */
    ...
    public static boolean isPostcode(String str) {
        ...
        return !(str.length() != 6 || !CNValidatorUtil.isNumber(str));
    ...
    //手机号表达式 带虚拟号
    private final static Pattern phone_pattern_with_virtual = Pattern.compile("^(13|15|16|17|18)\\d{9}$");
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
     ...
     * 验证手机号是否正确
     *
     * @param phone 手机号码 virtual 是否带167 170这种虚拟号
     * @return boolean
     */
    public static boolean isPhone(String phone, boolean virtual) {
        if (virtual) {
            return phone_pattern_with_virtual.matcher(phone).matches();
        } else {
            return phone_pattern.matcher(phone).matches();
        }
    }
