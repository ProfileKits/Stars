 ...
 * 万能recyclerView适配器
 */
public abstract class CNBaseAdapter<T> extends RecyclerView.Adapter {
    //正常type 0;
    ...
    private CNAnimationType CNAnimationType;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        if (viewType <= TYPE_FOOT) {
            return new CNHeadHolder(footViews.get(Math.abs(viewType + Math.abs(TYPE_FOOT))));
        } else if (TYPE_FOOT < viewType && viewType <= TYPE_HEAD) {
            return new CNHeadHolder(headViews.get(Math.abs(viewType + Math.abs(TYPE_HEAD))));
        } else {
    ...
    private void addItemAnimation(RecyclerView.ViewHolder viewHolder, int position) {
        if (animResId != 0 || CNAnimationType != null) {
            if (!alwaysShow) {
                if (position > currentPosition) {
                    ...
                    if (animResId != 0) {
                        ...
                        if (CNAnimationType != null) {
                            animator = AnimationUtils.loadAnimation(viewHolder.itemView.getContext(), CNAnimationType.getResId());
                        }
                    }
    ...
    public void onViewAttachedToWindow(@NonNull RecyclerView.ViewHolder holder) {
        ...
        if (lp != null && lp instanceof StaggeredGridLayoutManager.LayoutParams
                && holder instanceof CNHeadHolder) {
    ...
    public void showItemAnim(CNAnimationType CNAnimationType) {
        this.CNAnimationType = CNAnimationType;
    }

    public void showItemAnim(CNAnimationType CNAnimationType, boolean alwaysShow) {
        this.CNAnimationType = CNAnimationType;
